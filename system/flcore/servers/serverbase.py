...
class Server(object):
    def __init__(self, args, times):
        ...
        # self.attack_clients = args.adversaries_num # attack number
    ...
    def evaluate_fair(self, count):  # 服务器端评估公平性
        ...
        if self.algorithm == "FedFP":
            ...
            self.error.append(accuracy_score(self.global_model(torch.tensor(X_test.values, dtype=torch.float32).to(self.device)).argmax(dim=1).cpu().numpy(),Y_test)) 
            self.disc.append((max(self.measure_EO(Y_test,A_test,self.global_model(torch.tensor(X_test.values, dtype=torch.float32).to(self.device)).argmax(dim=1).cpu().numpy() ,0),
                        self.measure_EO(Y_test,A_test,self.global_model(torch.tensor(X_test.values, dtype=torch.float32).to(self.device)).argmax(dim=1).cpu().numpy() ,1))))
            y_pred = self.global_model(torch.tensor(X_test.values, dtype=torch.float32).to(self.device)).detach().cpu().numpy()
        metrics = {
                "accuracy": accuracy_score,
                "precision": precision_score,
                "false positive rate": false_positive_rate,
        ...
        # y_pred = self.global_model(torch.tensor(X_test.values, dtype=torch.float32).to(self.device)).cpu().numpy()
        ...
        # figure.savefig(f'runs/experiment_{self.exn}/g-{count}-fairness_metric_plot.png', dpi = 300)
        print(metrics)
